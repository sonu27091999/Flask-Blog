...
def admin_only(f):
    ...
    def wrapper(*args, **kwargs):
        # if current user id is not 1 abort
        if current_user.id != 1:
            return abort(403)
        # Otherwise continue with route function
        return f(*args, **kwargs)
     # Renaming the function name:  (stackoverflow:- https://stackoverflow.com/questions/17256602/assertionerror-view-function-mapping-is-overwriting-an-existing-endpoint-functi)
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        # Email doesn't exist
        if not user:
            flash('That email does not exist. plz try again')
            return redirect(url_for('login'))
        # Check stored password hash against entered password hashed.
            ...
            return redirect(url_for('get_all_posts'))
        # Email exists and password incorrect
        else:
            flash('password is wrong. plz try again')
            return redirect(url_for('login'))
